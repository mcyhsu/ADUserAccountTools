...
function Disable-Users {
    ...
    foreach($employee in $AccountsToDisable) {
        ...
        try {
            ...
            $UserExists = if(Get-ADUser -Identity $UserReference -ErrorAction SilentlyContinue) { $true } else { $false }
            $IsEnabled = if($(Get-ADUser -Identity $UserReference -ErrorAction SilentlyContinue).Enabled -eq $true) { $true } else { $false }
            ...
            # If there was an error in the previous steps, i.e. a user doesn't exist or the admin lacks permission, then display this warning message
            else {
